...
<?php
        $query_total_revenue = "SELECT IFNULL(sum(`total_amount`),0) FROM `invoice` WHERE `invoice_status`='Processed'";
        $result_total_revenue = mysqli_query($db, $query_total_revenue);
            if (!$result_total_revenue) {
                printf("Error: %s\n", mysqli_error($db));
                exit();           }
            $count = mysqli_num_rows($result_total_revenue);
            $row_total_revenue = mysqli_fetch_array($result_total_revenue);
?>
                                    <h2><?php echo $row_total_revenue[0]; ?></h2>
                    ...
                    <div class="col-md-3">
                        <div class="card p-30">
                            <div class="media">
                                ...
                                <div class="media-body media-text-right">
<?php
        $query_sales = "SELECT IFNULL(count(`invoice_id`),0) FROM `invoice` WHERE `invoice_status`='Processed'";
        $result_sales = mysqli_query($db, $query_sales);
            if (!$result_sales) {
                printf("Error: %s\n", mysqli_error($db));
                exit();           }
            $count_sales = mysqli_num_rows($result_sales);
            $row_sales = mysqli_fetch_array($result_sales);
?>
                                    <h2><?php echo $row_sales[0]; ?></h2>
                    ...
                    <div class="col-md-3">
                        <div class="card p-30">
                            <div class="media">
                                ...
                                <div class="media-body media-text-right">
<?php
        $query_stores = "SELECT IFNULL(count(`store_id`),0) FROM `stores`";
                    ...
                    <div class="col-md-3">
                        <div class="card p-30">
                            <div class="media">
                                ...
                                <div class="media-body media-text-right">
<?php
        $query_customers = "SELECT IFNULL(count(`cust_id`),0) FROM `customer`";
        $result_customers = mysqli_query($db, $query_customers);
            if (!$result_customers) {
                printf("Error: %s\n", mysqli_error($db));
                exit();
            }
            $count_customers = mysqli_num_rows($result_customers);
            $row_customers = mysqli_fetch_array($result_customers);
?>
                                    <h2><?php echo $row_customers[0]; ?></h2>
...
<?php 
        ...
        $datarow_recent_orders = "";
        $date_list = date('Y-m-d');
        $query_list = "SELECT `invoice_id`, `cust_id`,  `no_of_products`, `total_amount`, `invoice_status` FROM `invoice` where `invoice_date` = '2018-08-16'";
        $result_list = mysqli_query($db, $query_list);
            if (!$result_list) {
                printf("Error: %s\n", mysqli_error($db));
                exit();           }
        $count_recent_orders = mysqli_num_rows($result_list);
        if($count_recent_orders !== 0){
            while($row_list = mysqli_fetch_array($result_list)){
                
                $query_cust = "SELECT `cust_name` FROM `customer` where `cust_id` = '$row_list[1]'";
                ...
                $datarow_recent_orders = $datarow_recent_orders."<tr><td><a style=\"text-decoration: underline;\" href=\"#\">$row_list[0]</a></td><td>$row_cust[0]</td><td>$row_list[2]</td><td>$row_list[3]</td><td><span class=\"badge badge-success\">$row_list[4]</span></td></tr>";
            }
        }
?>
                <div class="row">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <?php echo $datarow_recent_orders ?>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
